...
class MetaICLData(object):
    ...
    def tensorize_before(self, num_class, _train_data, _test_data, options=None):

        train_data, test_data = [], []
        if self.use_demonstrations:
            zeros_list = [0] * num_class
            for dp in _train_data:
                dp_dict = {"input": dp[1], "output": dp[2]}
                assert type(dp_dict)==dict, ("Each example should be a dictionary", dp)
                assert "input" in dp_dict and "output" in dp_dict, ("Training example should contain input and output", dp)
                assert self.k % num_class == 0
                for j in range(len(options)):
                    if (dp[2] == options[j]):
                        zeros_list[j] = zeros_list[j] + 1

                    if (zeros_list[j]<=self.k/num_class and dp[2] == options[j]):
                        train_data.append(dp_dict.copy())
                if (len(train_data)>=self.k):
                    break
        random.shuffle(train_data) 
        for dp in _test_data:
            dp_dict = {"input": dp[1], "options": dp[3], "output": dp[2]}
            assert type(dp_dict)==dict, ("Each example should be a dictionary", dp)
            assert "input" in dp_dict and "options" in dp_dict and type(dp_dict["options"])==list, \
                ("Test example should contain input and options in a list format", dp)
            if "output" not in dp_dict:
                dp_dict["output"] = dp_dict["options"][0] # randomly choose one (we don't need it anyways)
            test_data.append(dp_dict.copy())
