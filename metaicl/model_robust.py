...
class MetaICLModel(object):
    ...
    def load(self, checkpoint=None, gpt2="gpt2-large"):
        ...
        if checkpoint is None:
            ...
            if gpt2.startswith("gpt2"):
                model = AutoModelForCausalLM.from_pretrained("", output_hidden_states=True, cache_dir="cached") 
            elif "opt" in gpt2:
                model = AutoModelForCausalLM.from_pretrained("", cache_dir="cached",
                    torch_dtype=torch.float16)
            elif "gpt-j" in gpt2:
                model = GPTJForCausalLM.from_pretrained("", output_hidden_states=True, cache_dir="cached", 
                    revision="float16", torch_dtype=torch.float16, low_cpu_mem_usage=True)
            elif "gpt-neo-" in gpt2:
                model = GPTNeoForCausalLM.from_pretrained("", cache_dir="cached")
            else:
    ...
    def do_inference(self, rob_bound, data, batch_size=1, verbose=False):
        ...
        for batch in dataloader:
            ...
            # with torch.no_grad():
            prob = self.run_model(rob_bound, input_ids, attention_mask, token_type_ids, labels=labels)
    ...
    def do_predict(self, data, batch_size=1, probs=None, verbose=False):
        ...
        for idx, dp in enumerate(data.metadata):
            ...
            prediction_idx = sorted(enumerate(curr_label_prob), key=lambda x: x[1], reverse=True)[0][0] 
    ...
    def run_model(self, rob_bound, input_ids, attention_mask, label_position, labels=None):
        with torch.no_grad():
        ...
        logits = torch.matmul(last_hidden_state, lm_head.weight.t())
        ...
        labels1 = input_ids
        labels1 = labels1[..., 1:].contiguous()

        loss_fct = torch.nn.CrossEntropyLoss()

        losses = loss_fct(logits.view(-1, logits.size(-1)),
                      labels1.view(-1)) 
        feature_grads = torch.autograd.grad(losses, last_hidden_state, retain_graph=True)[0]

        adv_feature = last_hidden_state + rob_bound * feature_grads

        adv_logits = torch.matmul(adv_feature, lm_head.weight.t())

        # shift
        adv_logits = adv_logits[..., :-1, :].contiguous() 
        label_position = label_position -1 # [bs]
        bs = len(adv_logits)
        _adv_logits = adv_logits[range(bs), label_position] 
        adv_prob = _adv_logits[:, labels[0]].view(-1)       
